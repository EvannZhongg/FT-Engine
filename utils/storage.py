...
class StorageManager:
    ...
    def log_data(self, group_name, ref_index, contestant_name, score_data, event_details):
        """
        记录数据到单独的 CSV
        :param score_data: {'total': int, 'plus': int, 'minus': int} (计算后的最终分)
        :param event_details: {'role': str, 'type': int, 'timestamp': int}
        """
        if not self.current_project_path: return
        ...
        # 如果文件不存在，写入表头 (移除了 Contestant 列，因为文件名已包含)
    ...
    def load_report_data(self, dir_name):
        ...
        for group_name in os.listdir(project_path):
            ...
            for file in os.listdir(group_path):
                ...
                # 解析文件名: PlayerName_Ref{index}.csv
                # 使用 rsplit 确保即使选手名中有下划线也能正确分割
