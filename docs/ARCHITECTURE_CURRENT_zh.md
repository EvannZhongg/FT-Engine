...
3. 扫描链路已在 Main IPC 归一化为 clone-safe `devices/errors` DTO；Renderer 在发送前会把 Pinia 响应式设备备注复制为普通对象，避免 `DataCloneError` 被误报为扫描失败。
4. 已完成 Windows 实机 BLE/USB 扫描与连接验证；仍需睡眠恢复、OBS、YouTube 网络和 macOS 签名/权限验收。
...
- `python -m unittest discover -s tests`：18/18 通过，仅包含 Platform Worker。
- `npm run build`、`npm run build:worker:win` 和 `npm run build:unpack`：通过。
- Windows 实机 Platform Worker 检查：`system.hello`、两台 BLE 扫描、双 BLE 连接、USB 识别、reset、通知和断开均成功；扫描返回普通 JSON DTO，设备状态和 counter 事件可正常回传。
- 解包版进程级检查：SQLite 和 Worker 握手成功，设备绑定页可显示两台 BLE 与 USB 设备，进程树无 TCP 监听；资源目录只有 `local-platform-worker.exe`，没有 backend、server 或端口配置。
- Node 测试覆盖 Competition/Stage/attempt、自由模式 SQLite 全链路 graph、会话转换回滚与审计、计分、干净 Schema 重建、上下文拒绝、导出格式、文件错误映射、扫描 DTO 序列化、Router 壳层、Renderer Store 分域边界和共享计分展示模型；仍缺赛事/复盘关键交互的浏览器自动化覆盖。
...
当前结果不代表睡眠恢复、OBS、YouTube 网络、macOS 权限/签名或 Windows 安装器已经完成发布验收；自由模式、BLE/USB 和扫描 DTO 已有代码级及 Windows 实机回归证据。
