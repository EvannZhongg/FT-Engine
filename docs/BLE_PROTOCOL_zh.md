...
语言：[English](BLE_PROTOCOL.md) | [中文](BLE_PROTOCOL_zh.md)

本文档描述计数器设备（Counter Device）的蓝牙低功耗（BLE）通信接口规范，适用于 Android、iOS、Windows 或 Web Bluetooth 客户端开发。
...
| :--- | :--- | :--- | :--- |
| **Service UUID** | `025018d0-6951-4a81-de4f-453d8dae9128` | - | 主服务 |
| **Characteristic** | `025018d0-6951-4a81-de4f-453d8dae9128` | Read, Notify, **Write** | **核心数据通道**（双向） |
| **Device Name** | `00002a00-0000-1000-8000-00805f9b34fb` | Read | 标准设备名特征（心跳检测） |

注意：
- 该特征值支持 **Notify**，建议连接后立即订阅以实时获取按键事件。
- 该特征值支持 **Write**，用于接收客户端控制指令（如重置）。
...
| **4** | `event_type` | `int8` | 1 Byte | **触发事件类型**<br>当前固件约定：`1`=加分、`-1`=减分、`0`=重置 |
...
> `event_type` 的具体含义以设备固件为准，若固件版本变化请同步更新定义。

### 2.3 客户端下行指令（Client -> Device）

客户端可通过向 Characteristic 写入（Write）特定指令控制设备。建议使用 Write Request（With Response）。

| 指令名称 | 载荷 (Hex) | 长度 | 功能说明 |
| :--- | :--- | :--- | :--- |
| 远程重置 (Reset) | 0x01 | 1 Byte | 将当前计数、累计计数全部归零，并重置状态。设备执行后通常会推送一次 `event_type = 0` 的数据包。 |

## 3. 开发示例代码

以下提供不同平台解析 17 字节数据的核心代码片段。
...
1. **扫描过滤**：建议扫描时通过 **Service UUID** (`025018...`) 过滤，或匹配设备名 `Counter-` 前缀。
2. **订阅通知**：连接建立并发现服务后，必须启用 Characteristic 的 Notification（写入 CCCD），否则无法实时接收按键事件。
3. **心跳/保活**：长时间无数据时（如 5-10 秒），读取一次标准 `Device Name` 特征或检查 RSSI 以确认连接仍有效。
